...
to add-links
  if any? turtles with [ count link-neighbors < max-degree ] [
    ask turtles [
      let my-neighbors count link-neighbors
      if my-neighbors < max-degree [
        let my-new-neighbor one-available-node
        set node1 self
        set node2 my-new-neighbor
        if my-new-neighbor != nobody [
          create-link-with my-new-neighbor [ set color green ]
        ]
      ]
    ]
  ]
